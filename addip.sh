...
adminip=$( curl -sS https://raw.githubusercontent.com/kenDevXD/izinn/main/accessmenu | grep $MYIP )
...
setadmin() {
#isi link git
linkinstall="https://raw.githubusercontent.com/kenDevXD/XRAY-MULTIPORT/main"
rm -rf /etc/adminip/
mkdir -p /etc/adminip
...
sleep 1
cek=$( curl -sS https://raw.githubusercontent.com/kenDevXD/izinn/main/ipvps.conf | awk '{print $5}' | grep $daftar )
if [[ $daftar = $cek ]]; then
...
git config --global user.name "kenDevXD"
git clone https://github.com/kenDevXD/izinn.git
mkdir /root/izinn
cd /root/izinn/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/kenDevXD/izinn.git
git push -f https://${tokengit}@github.com/kenDevXD/izinn.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/kenDevXD/XRAY-MULTIPORT/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
